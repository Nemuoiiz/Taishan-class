...
$('#navbar .main-link, .backtop a').each(function (index, link) {
	$(this).on('click', function (e) {
		e.preventDefault()	// 阻止 a 連結預設的動作
		if ($(this).attr('herf') == '#section04' || $(this).attr('href') == '#section05') {
			gsap.to(window, {
				// 指定滑動
				scrollTo: {
					y:`#section0${index + 1}`
...
$('.main-link').each(function (index, link) {
	...
	gsap.to(link, {
		scrollTrigger: {
			...
			toggleClass: {
				targets: link,
				className:'active',		// .active 有設定 css 黃色字
			},
			// markers: true,
		}
	})
...
$('.fog').each(function (index, fog) {
	gsap.set(fog, {
		...
		// x: function (a, b, c) {
		x: function () {
			// console.log(a,b,c)		// 取出數值後得到 index、自己本身元素、參與的陣列
			// console.log(index)	
...
function setStarTween(stars) {
	...
	stars.forEach(function (star, index) {
		gsap.set(star, {
			transformOrigin: '0 50%',		// 以左邊中央為基準點 (類似時鐘)
			position: 'absolute',
			// 假設 window 寬度 1000
			// 1000/2 = 500
			// 1000*2 = 2000
...
splitTexts.forEach(function (splitText) {
	...
	tl.from(
		...
		}, '+=3'	// 下一組動畫必須延遲 3s 才能進場，因為前一組離場動畫有設定延遲 3s
	)
})
