...
export const login = (req, res, next) => {
  ...
  // session: false 表示停用 cookie
  // (error, user, info) 對應 passport.js 的 done() 的三個東西
  passport.authenticate('login', { session: false }, (error, user, info) => {
    console.log(error, user, info)
    // 如果沒有收到資料 (第二參數user) 或發生錯誤就執行以下
    if (!user || error) {
      // Local 驗證策略的錯誤，缺少指定欄位的資料 (passport.js 內指定接收的 account 和 password)
